...
<table>
    <tbody>
        ...
        <tr>
            <td><img src="https://user-images.githubusercontent.com/68424403/210642206-1dccdf2e-73ea-4aa1-bfb5-56352e812844.gif"
                  width="430px"  alt=""></td>
            <td>회원가입 및 프로필 설정
              <ul>
                <li>이메일과 비밀번호를 설정하여 회원가입을 할 수 있습니다.</li>
                <li>유효성 검사를 진행하고, 오류 메시지를 전달합니다.</li>
                <li>(형식에 맞지 않는 이메일 주소, 6자 이하의 비밀번호 입력시 경고)</li>
                <li>올바른 이메일과 비밀번호를 입력하면 회원가입 버튼이 활성화 됩니다.</li>
                <li>회원가입이 완료되면 프로필 설정화면으로 이동합니다.</li>
                <li>유효성 검사를 진행하고, 오류 메시지를 전달합니다..</li>
                <li>(사용자의 이름은 2~10자를 벗어나면 경고)</li>
                <li>(계정 ID는 영문/숫자/특수문자(.),(_)외의 것을 입력시 경고)</li>
                <li>올바른 사용자 이름과 계정 ID를 입력하면 버튼이 활성화 됩니다.</li>
              </ul>
            </td>
        </tr>
        <tr>
...
width="430px"                    alt=""></td>
        ...
        <tr>
            <td><img src="https://user-images.githubusercontent.com/68424403/210638067-7de5780e-7d6b-4238-ba0a-f4b15e0da28e.gif"
                 width="430px"   alt=""></td>
            <td>프로필 수정하기<ul>
                    <li>사용자의 사진, 이름, ID, 소개를 수정할 수 있습니다.</li>
                    <li>사용자의 이름과, ID에 대한 유효성 검사를 수행합니다.</li>
                    <li>수정을 다하고 나서 버튼을 클릭하면 프로필 페이지로 돌아갑니다.</li>
                </ul>
            </td>
        </tr>
    </tbody>
...
#### :pushpin: pure CSS vs styled-components
...
### 2) 커스텀 훅 안에서 커스텀 훅을 사용할 때 나타나는 오류

- 문제상황
  - useDelete 커스텀 훅 안에 useGetCommentList 커스텀 훅을 넣어 사용했는데 불러온 함수가 원하는 로직대로 작동하지 않는다.
- 원인추론
...
- 이미지 압축<br>
  => 이미지 리사이징을 위한 browser-image-compression 라이브러리 사용하기<br>
...
- 지도 API 받아와서 지도 보여주는 기능 추가 [+,-를 이용해서 확대/축소 하는 기능추가]
...
- 스마트폰으로 사진을찍으면 표시되어있는 위도/경도 정보를 받아와서 처리
...
- 프로젝트 시작하기 전엔 마냥 긴장되고 두려웠지만 막상 진행하고 나니 팀원분들에게 많은 것을 배울 수 있는 시간이었습니다. 또한 하나의 기능을 구현하고 풀리지 않던 에러를 해결하는 즐거움을 느낄 수 있었습니다. 커스텀 훅을 처음 사용하면서 클로저의 개념을 제대로 익히지 못해 원하던 기능을 구현하지 못한 경험이 있습니다. 이 과정에서 스코프, 클로저의 개념을 제대로 익혀 코드를 수정하고 오류를 해결했습니다. 만들면서 공부하는 게 효율성이 높다는 말은 익히 들어왔지만 직접 경험해보니 그 효과를 제대로 인지할 수 있었습니다. 많이 부족하지만 더 많은 서비스를 만들어보며 지금의 부족함을 채워나가 개발자로서 더욱 성장하고 싶습니다.
